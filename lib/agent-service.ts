...
type AgentRow = {
  ...
  inft_contract_address: string | null;
  inft_mint_tx_hash: string | null;
  inft_owner_address: string | null;
};
...
type RunRow = {
  ...
  compute_mode: string;
  run_uri: string | null;
  run_root_hash: string | null;
  run_tx_hash: string | null;
  archive_status: "pending" | "archived" | "failed" | "skipped";
...
function mapAgent(row: AgentRow): AgentRecord {
  ...
  return {
    ...
    inftContractAddress: row.inft_contract_address,
    inftMintTxHash: row.inft_mint_tx_hash,
    inftOwnerAddress: row.inft_owner_address,
  };
}
...
function mapRun(row: RunRow): RunRecord {
  return {
    ...
    createdAt: row.created_at,
    runUri: row.run_uri,
    runRootHash: row.run_root_hash,
    runTxHash: row.run_tx_hash,
...
export async function updateAgent(input: {
  ...
  return withWrite(async (db) => {
    ...
    const row = await queryOne<AgentRow>(db, "SELECT * FROM agents WHERE id = ?", [input.id]);
    if (!row) {
...
export async function applyINFTMint(params: {
  agentId: number;
  tokenId: number;
  ...
  return withWrite(async (db) => {
    ...
    const row = await queryOne<AgentRow>(db, "SELECT * FROM agents WHERE id = ?", [params.agentId]);
    if (!row) throw new Error("Agent not found after INFT mint");
    return mapAgent(row);
  });
}

export async function listAgentsMissingINFT(): Promise<AgentRecord[]> {
  return withRead(async (db) => {
    const rows = await queryAll<AgentRow>(
      db,
      `
        ...
        WHERE manifest_uri IS NOT NULL
          AND storage_hash IS NOT NULL
          AND inft_token_id IS NULL
        ORDER BY id ASC;
      `,
      [],
    );
...
export async function getUserByEmail(email: string): Promise<UserRecord | null> {
  return withRead(async (db) => {
    const row = await queryOne<UserRow>(db, "SELECT * FROM users WHERE lower(email) = ?", [normalizeEmail(email)]);
    return row ? mapUser(row) : null;
  });
}
...
export async function getOrCreateEmailSessionUser(params: {
  ...
  return withWrite(async (db) => {
    ...
    const existing = await queryOne<UserRow>(db, "SELECT * FROM users WHERE lower(email) = ?", [email]);
    ...
    if (existing) {
      ...
      const updated = await queryOne<UserRow>(db, "SELECT * FROM users WHERE id = ?", [existing.id]);
    ...
    const row = await queryOne<UserRow>(db, "SELECT * FROM users WHERE id = ?", [getLastInsertId(db)]);
...
export async function getOrCreateWalletUser(walletAddress: string): Promise<UserRecord> {
  return withWrite(async (db) => {
    const existing = await queryOne<UserRow>(db, "SELECT * FROM users WHERE wallet_address = ?", [walletAddress]);
    if (existing) {
...
export async function authenticateEmailUser(params: {
  ...
  return withRead(async (db) => {
    const row = await queryOne<UserRow>(db, "SELECT * FROM users WHERE lower(email) = ?", [normalizeEmail(params.email)]);
...
export async function completeUserOnboarding(params: {
  ...
  return withWrite(async (db) => {
    ...
    const row = await queryOne<UserRow>(db, "SELECT * FROM users WHERE id = ?", [params.userId]);
...
export async function getRunById(runId: number): Promise<RunRecord | null> {
  return withRead(async (db) => {
    const row = await queryOne<RunRow>(db, "SELECT * FROM runs WHERE id = ?", [runId]);
    return row ? mapRun(row) : null;
  });
}

export async function markRunArchive(params: {
  runId: number;
  ...
  return withWrite(async (db) => {
    await db.run(
      ...
      [
        params.uri ?? null,
        params.rootHash ?? null,
        params.txHash ?? null,
        params.status,
        params.runId,
      ],
    );
    const row = await queryOne<RunRow>(db, "SELECT * FROM runs WHERE id = ?", [params.runId]);
    return row ? mapRun(row) : null;
  });
}
...
export async function runAgentForUser(params: {
  ...
  return withWrite(async (db) => {
    ...
    await db.run("UPDATE users SET credits = credits - ? WHERE id = ?", [price, params.userId]);
    await db.run(
      `
    ...
    if (agent.creator_id !== params.userId && price > 0) {
      await db.run("UPDATE users SET credits = credits + ? WHERE id = ?", [price, agent.creator_id]);
      await db.run(
        `
          INSERT INTO credit_ledger (user_id, kind, amount, reference_type, reference_id, note)
...
export async function completeOnchainTopup(params: {
  ...
  return withWrite(async (db) => {
    ...
    await db.run("UPDATE users SET credits = credits + ? WHERE id = ?", [params.amount, params.userId]);
    await db.run(
      `
        INSERT INTO credit_ledger (user_id, kind, amount, reference_type, reference_id, note)
...
export async function reconcileTopupOrder(params: {
  ...
  return withWrite(async (db) => {
    ...
    if (params.status === "completed") {
      await db.run("UPDATE users SET credits = credits + ? WHERE id = ?", [order.credits, order.user_id]);
      await db.run(
        `
          INSERT INTO credit_ledger (user_id, kind, amount, reference_type, reference_id, note)
          VALUES (?, 'topup', ?, 'topup_order', ?, ?);
...
export async function getCreditStats(userId: number): Promise<{
  ...
  return withRead(async (db) => {
    ...
    const creatorEarnedRow = await queryOne<{ total: number | null }>(
      db,
