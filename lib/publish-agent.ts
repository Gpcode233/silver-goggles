...
import { mintINFTForAgent } from "@/lib/zero-g/inft";
import {
...
export async function publishAgentWithOptions(
  ...
  // Fire-and-forget: mint the iNFT once the manifest is durable on 0G Storage.
  // Only attempt if the contract is configured. Failures are non-fatal — the
  // agent is still published and listAgentsMissingINFT will pick it up later.
  if (process.env.INFT_CONTRACT_ADDRESS) {
    void mintINFTForAgent(agentId).catch((error) => {
      console.warn(`[inft] mint failed for agent ${agentId}:`, error instanceof Error ? error.message : error);
    });
  }
